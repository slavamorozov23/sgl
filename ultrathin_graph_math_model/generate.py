    ...
    with torch.no_grad():
        ...
        with progress:
            for i in range(max_new_tokens):
                ...
                input_token_ids_step = generated_ids[:, -1:]

                logits, *_ = model(input_ids=input_token_ids_step)
                ...
                if config.REPETITION_PENALTY > 1.0:
                    previous_tokens = generated_ids[0]
                    if config.REPETITION_WINDOW > 0:
                        previous_tokens = previous_tokens[-config.REPETITION_WINDOW:]
                    unique_prev = torch.unique(previous_tokens)
                    for tok_id in unique_prev:
                        logit_val = next_token_logits[0, tok_id]
                        if logit_val > 0:
                            next_token_logits[0, tok_id] = logit_val / config.REPETITION_PENALTY
                        else:
                            next_token_logits[0, tok_id] = logit_val * config.REPETITION_PENALTY
